...
nyc_crime_2017 %>%
  ...
  group_by(hour, boro) %>%
  summarize(crime_count = n()) %>%
  ggplot(aes(x = hour, y = crime_count, color = boro)) + geom_point(alpha = 0.5) + geom_line()
```

Make a plot showing the crime counts and crime rate based each months
```{r, echo=FALSE}
...
Build a function to get the most prevalent crimes in each boro and illustrate the result.
