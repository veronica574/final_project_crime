...
In addition, we have a strong interest in finding potential factors that may associated with criminal rate. In this case, we firstly choose household income level by borough. After reading data from the web, data cleaning and data visualization, we are surprized to see from the scatter plot: Both lower-income borough and higher-income borough have an extremely high crime rate.
...
## Assoiciation between crime and unemployment
...
```{r}
unemployment_crime %>%
  ggplot(aes(x = unemployment_rate, y = crime_rate, color = unemployment_rate)) + geom_point(alpha = 0.5) + geom_line() +
  labs(title = "Corelation between unemployment rate and crime rate in each borough",
       x = "Unemployment rate",
       y = "Crime rate")
```
